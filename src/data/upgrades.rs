...
impl UpgradeDef {
    pub fn cost_at_level(&self, level: u32) -> u64 {
        let raw = self.base_cost as f64 * self.cost_scaling.powi(level as i32);
        // Round to nearest "nice" number based on magnitude
        let magnitude = 10f64.powf((raw.log10().floor()).max(0.0));
        let rounded = (raw / magnitude).ceil() * magnitude;
